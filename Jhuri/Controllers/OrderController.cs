...
namespace Jhuri.Controllers
{
    public class OrderController : Controller
    {
        ...
        public OrderController(UserManager<ApplicationUsers> userManager, IUnitOfWork unitOfWork)
        {
        ...
        {
            List<OrderDetailsViewModel> mode = new List<OrderDetailsViewModel>();
            ...
            if (addToCartList != null)
            {
                addToCartList.ForEach(c =>
               {
                   ...
                   if (product != null)
                   {
                       ...
                       orderDetails.ImagePath = product.ProductImages.FirstOrDefault(x => x.ProductId == c.ProductId)?.ImagePath;

                       var pStock = product.ProductStocks.First(x => x.ProductId == c.ProductId);
                       if (pStock != null)
                       {
                           orderDetails.Stock = (pStock.InQuantity - pStock.OutQuantity) < 1 ? 0 : pStock.InQuantity - pStock.OutQuantity;
                       }
                       mode.Add(orderDetails);
                   }
               });
            }
        ...
        public IActionResult Index(IEnumerable<OrderDetailsViewModel> product)
        {
            ...
            HttpContext.Session.Set<List<AddToCartViewModel>>("CartItem", modelCart);
            itemInCart =
            HttpContext.Session.Get<List<AddToCartViewModel>>("CartItem").Count;
